...
export default function SpeedTest() {
  ...
  const [details, setDetails] = useState<Measurement | null>(null);
  ...
  // Hidden tabs throttle timers and callbacks, so their measurements are discarded.
  useEffect(() => {
    const visibilityChanged = () => {
      if (document.visibilityState === 'hidden') controller.current?.abort(new Error('Test stopped because this tab was hidden. Keep it visible and run again for reliable timing.'));
    };
  ...
  async function start() {
    ...
    if (document.visibilityState === 'hidden') { setError('Keep this tab visible while the test runs.'); return; }
    ...
    try {
      ...
      setPhase('done'); setDetails(result); setSpeed(result.download);
      ...
      if (run.signal.aborted) { setPhase('cancelled'); if (run.signal.reason instanceof Error && run.signal.reason.name !== 'AbortError') setError(run.signal.reason.message); }
  ...
  return <div className="page"><Link href="/" className="back-link"><ArrowLeft size={14}/>All tools</Link><div className="tool-heading"><span className="tool-icon"><Gauge size={28}/></span><div><h1>Speed test</h1><p>A fuller picture of speed and responsiveness.</p></div></div>
    ...
    {details && <div className="my-5 rounded-panel border border-border bg-bg p-4 text-left">
      ...
      <dl className="grid grid-cols-2 gap-4 text-sm">
        ...
        <div><dt className="text-dim">Measurement</dt><dd className="mt-1 font-medium text-text">Median of 3 sustained rounds</dd></div>
        ...
        <div><dt className="text-dim">Download range</dt><dd className="mt-1 font-medium text-text">{details.downloadRange?.min.toFixed(1)}–{details.downloadRange?.max.toFixed(1)} Mbps</dd></div>
        <div><dt className="text-dim">Upload range</dt><dd className="mt-1 font-medium text-text">{details.uploadRange?.min.toFixed(1)}–{details.uploadRange?.max.toFixed(1)} Mbps</dd></div>
      </dl>
