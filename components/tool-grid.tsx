...
export default function ToolGrid() {
  ...
  return <>
    ...
    <div className="mb-6 flex items-center gap-3 rounded-panel border border-border bg-panel px-4 py-3 text-dim"><label className="flex min-w-0 flex-1 items-center gap-3"><Search size={18}/><span className="sr-only">Search tools</span><input type="search" className="w-full min-w-0 bg-panel text-text outline-none" placeholder="Search tools, calculators, images…" value={query} onChange={event => setQuery(event.target.value)}/></label><CommandTrigger compact/></div>
