...
export default function ToolGrid() {
  ...
  return <>
    ...
    <div className="tool-grid">{matches.map(t => <Link key={t.slug} href={`/tools/${t.slug}`} className="tool-card"><div className="card-top"><span className="tool-icon"><t.icon size={27} strokeWidth={1.7}/></span><ArrowUpRight size={21} className="card-arrow"/></div><span className="category">{t.category}</span><h3>{t.name}</h3><p>{t.description}</p><div className="card-bottom"><span>Open tool <ArrowRight size={15}/></span><span className="availability"><span className="status-dot"/>Ready to use</span></div></Link>)}
    {!query.trim() && <div className="coming-card"><span className="coming-icon"><Plus size={25} strokeWidth={1.4}/></span><h3>A little room for more.</h3><p>More everyday tools will find<br/>their home here.</p><span className="coming-label">THE TOOLKIT IS GROWING</span></div>}</div>
