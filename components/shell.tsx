...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell"><CommandPalette/>
    ...
    <div className="main-shell"><header className="header"><div className="breadcrumb"><span>Workspace</span><span className="slash">/</span><span>{current?.name ?? (path === '/' ? 'All tools' : 'Page not found')}</span></div><div className="header-right">{path !== '/' && <CommandTrigger/>}<div className="theme-switch" role="group" aria-label="Color theme"><button type="button" className={!dark ? 'selected' : ''} onClick={() => setTheme('light')} aria-label="Light theme" aria-pressed={!dark}><Sun size={17}/></button><button type="button" className={dark ? 'selected' : ''} onClick={() => setTheme('dark')} aria-label="Dark theme" aria-pressed={dark}><Moon size={16}/></button></div></div></header>
    <main id="main">
